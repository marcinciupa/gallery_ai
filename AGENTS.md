# gallery_ai — project guide for Codex
...
- **⛔ NIGDY nie dodawać `MANAGE_EXTERNAL_STORAGE` („dostęp do wszystkich plików") — usunięte w 0.969.**
  Google Play odrzuciło je DWA RAZY (2026-09-02): (1) galeria nie jest na liście dozwolonych zastosowań
  (menedżer plików, backup, antywirus, zarządzanie dokumentami, wyszukiwanie plików, szyfrowanie dysku,
  migracja urządzeń), (2) apce operującej na samych zdjęciach polityka NAKAZUJE MediaStore. Uprawnienie nigdy
  nie odblokowywało tu żadnej funkcji — służyło WYŁĄCZNIE do wyciszenia systemowego okna zgody. Nie bronić go
  deklaracją: próba przemianowania apki na „menedżer plików" w opisie sklepowym była ślepą uliczką.
  Usunięte: `src/lib/allFilesAccess.ts`, nakładka „ALLOW FILE ACCESS?", zależność `expo-intent-launcher`,
  `store_assets/all_files_access_declaration.md`.
  - Kasowanie (`mediaOps.ts` → `deleteCore`): najpierw `File.delete()` po ścieżce — przechodzi BEZ OKNA dla
    plików, których właścicielem jest apka (kopie z COPY/MOVE, zapisy po edycji AI). Reszta przez
    `Asset.delete` → `MediaStore.createDeleteRequest` = JEDNO okno na całą paczkę.
  - Odmowa w oknie to NORMALNY wynik: `deleteAssets`/`deleteItems`/`deleteForever` zwracają klucze, które
    faktycznie zniknęły. Nigdy nie czyścić stanu apki bezwarunkowo. Przy MOVE odmowa bywa CZĘŚCIOWA — rollback
...
5. **Sekrety tylko w `.env` (gitignored)** — nigdy w kodzie/AGENTS.md/pamięci.
