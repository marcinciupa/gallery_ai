...
export function MenuBar({ items, index, focused, onPick }: { items: readonly string[]; index: number; focused: boolean; onPick: (i: number) => void }) {
      ...
      {items.map((label, i) =>
        i === index ? (
          <Pressable key={label} onPress={() => onPick(i)} hitSlop={HS} style={{ flexDirection: 'row', alignItems: 'center', gap: 3, paddingVertical: 3, paddingHorizontal: 4, borderRadius: 2, backgroundColor: pillBg, ...(focused ? null : (PILL as any)) }}>
            <Text style={{ ...txt, color: pillFg, ...(focused ? phosphorGlow : null) }}>{'•'}</Text>
            <Text style={{ ...txt, color: pillFg, ...(focused ? phosphorGlow : null) }}>{label}</Text>
          </Pressable>
        ) : (
          <Pressable key={label} onPress={() => onPick(i)} hitSlop={HS} style={{ paddingVertical: 3 }}>
            <Text style={{ ...txt, color: focused ? color.dark21 : screen.olive.primary, ...(focused ? null : phosphorGlow) }}>{label}</Text>
          </Pressable>
        ),
      )}
    </View>
