...
⚠️ Every feature bullet below was checked against the code (2026-09-04). Do not add a claim
...
`MediaStore.createDeleteRequest`, which shows one Android confirmation per batch.
...
Moving or deleting a photo that another app created (your camera, screenshots, downloads) uses Android's standard media flow, so Android asks you to confirm — once for the whole selection, not once per photo.
